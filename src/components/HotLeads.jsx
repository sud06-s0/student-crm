...
const HotLeads = ({ onLogout, user }) => {
  ...
  // ← UPDATED: Calculate stage counts using stage_key for HOT LEADS ONLY
  ...
  // ← UPDATED: Determine which data to display - FILTER FOR HOT LEADS ONLY
  const getDisplayLeads = () => {
    // First filter for hot leads only
  ...
  // ← Get hot leads count for header
  const hotLeadsCount = leadsData.filter(lead => lead.category === 'Hot').length;
            ...
            <tbody>
              {!loading && displayLeads.length > 0 ? (
                ...
                <tr>
                  ...
                  {searchTerm 
                    ? 'No hot leads found for your search.' 
