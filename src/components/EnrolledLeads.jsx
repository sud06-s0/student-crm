...
const EnrolledLeads = () => {
  ...
  // Real data from Supabase (filtered for enrolled leads only)
  const [leadsData, setLeadsData] = useState([]);
  const [lastActivityData, setLastActivityData] = useState({});

  // Filter states - UPDATED to match LeadsTable
  ...
  // Enrolled stages only for left sidebar display
  const enrolledStages = stages.filter(stage => 
    ['Admission'].includes(stage.value)
  );
  ...
  // Calculate stage counts for enrolled stages only
  ...
  // Fetch enrolled leads from Supabase (category = 'Enrolled')
  ...
  // Fetch enrolled leads on component mount
  useEffect(() => {
    fetchEnrolledLeads();
  }, []);
  ...
  const handleSidebarStageChange = async (leadId, newStage) => {
    try {
      ...
      // If the lead is no longer enrolled, remove from local state and close sidebar
      if (updatedCategory !== 'Enrolled') {
        const updatedLeads = leadsData.filter(lead => lead.id !== leadId);
        setLeadsData(updatedLeads);
        closeSidebar();
      } else {
        // Update local state if still enrolled
  ...
  const handleUpdateAllFields = async () => {
    try {
      ...
      // Check if lead will remain enrolled after update
      ...
      // If lead is no longer enrolled, close sidebar and refresh
      if (newCategory !== 'Enrolled') {
        closeSidebar();
        await fetchEnrolledLeads();
      } else {
        // Refresh the leads data
        await fetchEnrolledLeads(); // This will also refresh activity data
  ...
  const handleStageChangeFromDropdown = async (e, leadId, newStage) => {
    ...
    try {
      ...
      // If the lead is no longer enrolled, remove from local state
      if (updatedCategory !== 'Enrolled') {
        const updatedLeads = leadsData.filter(lead => lead.id !== leadId);
        setLeadsData(updatedLeads);
      } else {
  ...
  const handleAddLead = async (action = 'add') => {
    await fetchEnrolledLeads(); // This will also fetch activity data
    await fetchLastActivityData();
  ...
  const getDisplayLeads = () => {
    ...
    // Then apply filters - UPDATED to include statusFilters
      ...
      <div className="nova-main">
        ...
        <div className="nova-header">
          <div className="header-left">
            ...
            <span className="total-count">Total Enrolled Students {leadsData.length}</span>
          </div>
          <div className="header-right">
            ...
            {/* UPDATED: Added statusFilters props */}
            <FilterButton
        ...
        {loading && (
          <div className="loading-message">
            <Loader2 size={16} className="animate-spin" /> Loading enrolled students...
          </div>
        )}
              ...
              {!loading && displayLeads.length > 0 ? (
                ...
                <tr>
                  <td colSpan="9" className="no-data">
                    {searchTerm ? 'No results found for your search.' : 'No enrolled students available. Click + Add Lead to create your first lead!'}
                  </td>
                </tr>
              ) : null}
            </tbody>
